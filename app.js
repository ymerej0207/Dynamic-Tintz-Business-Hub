...
function quickShotOpenQuote(quote){
  ...
  if(/Cleaning$/.test(quote.project_type||''))return openCareQuickShot(quote);
...
function renderQuickShotPicker(){
  ...
  host.innerHTML=quotes.length?quotes.map(q=>{
    ...
    return `<button type="button" class="quickshot-quote-choice" data-quickshotquote="${q.id}"><span><b>${esc(name)}</b><small>${esc(q.project_name||'Window Film Project')}${address?` • ${esc(address)}`:''}</small></span><span><strong>${/Cleaning$/.test(q.project_type||'')?'Window care':sqft.toFixed(2)+' sq ft'}</strong><small>${esc(q.status||'')}</small></span></button>`;
...
async function previewQuickShot(){try{await buildQuickShotImage();toast('QuickShot preview ready.')}catch(e){alert(e.message)}}
async function shareQuickShot(){
  try{let blob=await buildQuickShotImage(),safe=($('qsCustomer')?.value||'Customer').trim().replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'')||'Customer',file=new File([blob],`Dynamic-Tintz-QuickShot-${safe}.png`,{type:'image/png'});
    if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({files:[file],title:'Dynamic Tintz QuickShot Quote'});return}
    let a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=file.name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),2000);toast('QuickShot picture saved.');
  }catch(e){if(e?.name!=='AbortError')alert(e.message||'Could not share the QuickShot picture.')}
}


// Window Care QuickShot is kept in memory. Editing it never changes the quote or Square draft.
let careQuickShotLines=[];
function openCareQuickShot(q){
 const fromQuote=!!q,lines=fromQuote?(Array.isArray(q.additional_services)?q.additional_services:[]):careLines();
 if(!lines.length)return toast('Select a cleaning package first.');
...
function renderCareQuickShot(){
 let host=$('cqsLines');if(!host)return;
 host.innerHTML=careQuickShotLines.map((l,i)=>`<div class="care-quickshot-line"><div><b>${esc(l.name)}</b><small>${esc(l.variation)}</small></div><label>Qty<input type="number" min="1" step="1" data-cqs-qty="${i}" value="${l.qty}"></label><label>Price $<input type="number" min="0" step="0.01" data-cqs-price="${i}" value="${l.price}"></label><strong>${money(careLineTotal(l))}</strong></div>`).join('');
 host.querySelectorAll('[data-cqs-qty],[data-cqs-price]').forEach(el=>el.onchange=()=>{let i=Number(el.dataset.cqsQty??el.dataset.cqsPrice),field=el.hasAttribute('data-cqs-qty')?'qty':'price';careQuickShotLines[i][field]=field==='qty'?Math.max(1,Math.floor(Number(el.value)||1)):Math.max(0,Number(el.value)||0);renderCareQuickShot()});
 let sum=careQuickShotLines.reduce((n,l)=>n+careLineTotal(l),0),savings=Math.min(sum,Math.max(0,Number($('cqsSavings').value)||0));
 $('cqsSubtotal').textContent=money(sum);$('cqsSavingsValue').textContent=money(savings);$('cqsTotal').textContent=money(sum-savings);
}
...
async function buildCareQuickShotImage(){
 ...
 let bg=ctx.createLinearGradient(0,0,1200,h);bg.addColorStop(0,'#020808');bg.addColorStop(.55,'#07191b');bg.addColorStop(1,'#020707');ctx.fillStyle=bg;ctx.fillRect(0,0,1200,h);ctx.strokeStyle='rgba(84,236,231,.34)';ctx.lineWidth=3;ctx.beginPath();ctx.roundRect(38,38,1124,h-76,42);ctx.stroke();
 try{let img=new Image();img.src='dynamic-tintz-mark.png';await img.decode();ctx.drawImage(img,72,72,150,150)}catch{}
 ctx.fillStyle='#fff';ctx.font='800 54px Arial,sans-serif';ctx.fillText('Dynamic Tintz',250,129);ctx.fillStyle='#8ff9f3';ctx.font='800 25px Arial,sans-serif';ctx.fillText('WINDOW CARE QUICKSHOT',252,176);ctx.fillStyle='#b6cacc';ctx.font='20px Arial,sans-serif';ctx.fillText('A fresh view for your home or business',252,215);
 ctx.textAlign='right';ctx.fillStyle='#fff';ctx.font='700 19px Arial,sans-serif';ctx.fillText(quickShotRef(),1120,104);ctx.fillStyle='#a9babc';ctx.fillText(new Date().toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric'}),1120,137);ctx.textAlign='left';
 ctx.fillStyle='#062326';ctx.beginPath();ctx.roundRect(72,270,1056,210,26);ctx.fill();ctx.fillStyle='#8ff9f3';ctx.font='800 18px Arial,sans-serif';ctx.fillText('PREPARED FOR',105,315);ctx.fillStyle='#fff';ctx.font='800 38px Arial,sans-serif';quickShotWrap(ctx,customer,105,364,925,43,2);ctx.fillStyle='#d3dfe0';ctx.font='21px Arial,sans-serif';quickShotWrap(ctx,[project,address].filter(Boolean).join(' • '),105,423,970,28,2);
 ctx.fillStyle='#8ff9f3';ctx.font='800 19px Arial,sans-serif';ctx.fillText('YOUR WINDOW CARE',90,543);let y=586;
 for(let l of careQuickShotLines){ctx.fillStyle='rgba(255,255,255,.045)';ctx.beginPath();ctx.roundRect(72,y-27,1056,88,14);ctx.fill();ctx.fillStyle='#fff';ctx.font='700 23px Arial,sans-serif';quickShotWrap(ctx,l.name,98,y+1,690,28,2);ctx.fillStyle='#b6cacc';ctx.font='18px Arial,sans-serif';ctx.fillText(l.variation||'Qty '+l.qty,98,y+36);ctx.textAlign='right';ctx.fillStyle='#8ff9f3';ctx.font='800 25px Arial,sans-serif';ctx.fillText(quickShotMoney(careLineTotal(l)),1095,y+16);ctx.textAlign='left';y+=105}
 y+=45;ctx.strokeStyle='rgba(255,255,255,.18)';ctx.beginPath();ctx.moveTo(90,y);ctx.lineTo(1110,y);ctx.stroke();y+=55;ctx.fillStyle='#d3dfe0';ctx.font='23px Arial,sans-serif';ctx.fillText('Services',100,y);ctx.textAlign='right';ctx.fillText(quickShotMoney(subtotal),1095,y);ctx.textAlign='left';
 if(savings){y+=49;ctx.fillStyle='#8ff9f3';ctx.fillText('Courtesy savings',100,y);ctx.textAlign='right';ctx.fillText('-'+quickShotMoney(savings),1095,y);ctx.textAlign='left'}
 y+=87;ctx.fillStyle='#8ff9f3';ctx.font='800 27px Arial,sans-serif';ctx.fillText('ESTIMATED TOTAL',100,y);ctx.textAlign='right';ctx.font='900 45px Arial,sans-serif';ctx.fillText(quickShotMoney(subtotal-savings),1095,y);ctx.textAlign='left';
 if(note){ctx.fillStyle='#d3dfe0';ctx.font='19px Arial,sans-serif';quickShotWrap(ctx,note,100,Math.min(h-225,y+70),990,29,3)}
 ctx.fillStyle='#9cb1b3';ctx.font='16px Arial,sans-serif';ctx.fillText('Preliminary visual estimate. Final scope and pricing confirmed before invoicing.',72,h-117);ctx.fillStyle='#fff';ctx.font='800 20px Arial,sans-serif';ctx.fillText('dynamictintz.com  •  469-840-4008  •  Locally owned in Anna, Texas',72,h-77);
 let blob=await new Promise(resolve=>canvas.toBlob(resolve,'image/png'));if(!blob)throw new Error('Could not create the picture.');let url=URL.createObjectURL(blob),img=$('careQuickShotPreview');if(img.dataset.url)URL.revokeObjectURL(img.dataset.url);img.src=url;img.dataset.url=url;$('careQuickShotPreviewWrap').classList.remove('hidden');return blob;
}
async function previewCareQuickShot(){try{await buildCareQuickShotImage();toast('Window Care QuickShot ready.')}catch(e){toast(e.message)}}
async function shareCareQuickShot(){try{let blob=await buildCareQuickShotImage(),safe=($('cqsCustomer').value||'Customer').replace(/[^a-z0-9]+/gi,'-'),file=new File([blob],`Dynamic-Tintz-Window-Care-${safe}.png`,{type:'image/png'});if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({files:[file],title:'Dynamic Tintz Window Care'});return}let a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download=file.name;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(a.href),2000);toast('QuickShot picture saved.')}catch(e){if(e?.name!=='AbortError')toast(e.message||'Could not share the picture.')}}
$('closeCareQuickShot').onclick=closeCareQuickShot;$('previewCareQuickShot').onclick=previewCareQuickShot;$('shareCareQuickShot').onclick=shareCareQuickShot;$('cqsSavings').oninput=renderCareQuickShot;
...
function openOrganicLeadModal(){['organicFirstName','organicLastName','organicPhone','organicEmail','organicAddress','organicNotes'].forEach(id=>{if($(id))$(id).value=''});$('organicSource').value='Google';$('organicService').value='Window Film';$('organicLeadMessage').textContent='';$('organicLeadModal').classList.add('show')}
async function saveOrganicLead(){let p={first_name:$('organicFirstName').value.trim(),last_name:$('organicLastName').value.trim(),phone:$('organicPhone').value.trim(),email:$('organicEmail').value.trim(),service_address:$('organicAddress').value.trim(),service_requested:$('organicService').value,notes:$('organicNotes').value.trim(),source:'organic',source_detail:$('organicSource').value,status:'new',created_at:new Date().toISOString(),updated_at:new Date().toISOString()};if(!p.first_name&&!p.last_name&&!p.phone&&!p.email){$('organicLeadMessage').textContent='Enter at least a name, phone, or email.';return}let{error}=await sb.from('leads').insert(p);if(error){delete p.source_detail;let r=await sb.from('leads').insert(p);if(r.error){$('organicLeadMessage').textContent=r.error.message;return}}$('organicLeadModal').classList.remove('show');$('leadStatusFilter').value='new';toast('Organic lead added.');await loadLeads()}
...
function careCustomerChosen(){
 ...
 if(!careCustomerId&&!careSquareCustomerId){box.classList.add('hidden');box.textContent='';return}
 box.classList.remove('hidden');
 box.innerHTML='<span>✓ '+esc(careCustomerId?'Existing OS customer linked':'Existing Square customer selected')+'</span><button type="button" id="careUnlinkCustomer" class="btn">Change</button>';
 $('careUnlinkCustomer').onclick=()=>{careCustomerId=null;careSquareCustomerId=null;careCustomerChosen();$('careCustomerSearch').focus()};
}
async function careFindCustomers(){
 const term=$('careCustomerSearch').value.trim(),seq=++careSearchSequence,results=$('careCustomerResults');
 if(term.length<3){results.replaceChildren();return}
 results.textContent='Searching customers…';
 let fields=['first_name','last_name','email','phone'],queries=fields.map(field=>sb.from('customers').select('id,first_name,last_name,email,phone,service_address').ilike(field,'%'+term.replace(/[%_]/g,'')+'%').limit(8));
 let digits=carePhoneDigits(term);if(digits.length>=7)queries.push(sb.from('customers').select('id,first_name,last_name,email,phone,service_address').ilike('phone','%'+digits.slice(-4)+'%').limit(12));
 let responses=await Promise.all(queries),local=[...new Map(responses.flatMap(r=>r.data||[]).map(c=>[c.id,c])).values()].filter(c=>!digits||digits.length<7||!c.phone||carePhoneDigits(c.phone).includes(digits.slice(-7))||[c.first_name,c.last_name,c.email].some(v=>String(v||'').toLowerCase().includes(term.toLowerCase()))).slice(0,8);
 let square=[],squareError='';
 if((term.includes('@')&&term.includes('.'))||digits.length===10){
  try{
   let{data,error}=await sb.functions.invoke('create-square-cleaning-draft',{body:{action:'search-customers',term}});
   if(error)throw error;square=data.customers||[];
  }catch(e){squareError='Square search unavailable. OS results still work.'}
 }
 if(seq!==careSearchSequence)return;
 const emails=new Set(local.map(c=>String(c.email||'').toLowerCase()).filter(Boolean)),phones=new Set(local.map(c=>carePhoneDigits(c.phone)).filter(Boolean));
 square=square.filter(c=>!(c.email&&emails.has(c.email.toLowerCase()))&&!(c.phone&&phones.has(carePhoneDigits(c.phone))));
 careCustomerMatches=[...local.map(c=>({...c,source:'os'})),...square.map(c=>({...c,source:'square'}))];
 results.innerHTML=careCustomerMatches.length?careCustomerMatches.map((c,i)=>'<button type="button" data-care-customer="'+i+'" class="care-customer-match"><b>'+esc([c.first_name,c.last_name].filter(Boolean).join(' ')||'Customer')+'</b><small>'+esc([c.email,c.phone,c.service_address].filter(Boolean).join(' · '))+'</small><span>'+esc(c.source==='os'?'Dynamic Tintz OS':'Square')+'</span></button>').join(''):'<div class="muted">No matching customer. Continue with a new customer.</div>';
 if(squareError)results.insertAdjacentHTML('beforeend','<small class="muted">'+esc(squareError)+'</small>');
 results.querySelectorAll('[data-care-customer]').forEach(button=>button.onclick=()=>{
...
async function ensureCareLead(customerId){
 ...
 if(r.error){let {customer_id,updated_at,...fallback}=lead;r=await sb.from('leads').insert(fallback).select('id').single()}
...
async function careSave(){
 ...
 try{
  let customer={first_name:name,last_name:$('careLast').value.trim(),email:$('careEmail').value.trim(),phone:$('carePhone').value.trim(),service_address:address,lead_source:$('careLeadSource').value,updated_at:new Date().toISOString()};
  let customerId=careCustomerId;
  if(!customerId){
   let matching=null,email=customer.email.toLowerCase(),phone=carePhoneDigits(customer.phone);
   if(email){let r=await sb.from('customers').select('id').ilike('email',email).limit(2);if(r.error)throw r.error;if(r.data?.length===1)matching=r.data[0]}
   if(!matching&&phone.length===10){let r=await sb.from('customers').select('id,phone').ilike('phone','%'+phone.slice(-4)+'%').limit(30);if(r.error)throw r.error;let exact=(r.data||[]).filter(x=>carePhoneDigits(x.phone)===phone);if(exact.length===1)matching=exact[0]}
...
function setupCareBuilder(){
 ...
 $('careCustomerSearch').addEventListener('input',()=>{clearTimeout(careSearchTimer);careSearchTimer=setTimeout(careFindCustomers,300)});
 for(const id of ['careFirst','careEmail','carePhone'])$(id).addEventListener('input',()=>{
  if(careCustomerId||careSquareCustomerId){careCustomerId=null;careSquareCustomerId=null;careCustomerChosen()}
...
async function syncCustomerLifecycle(customerId,status){
  if(!customerId)return {ok:true,count:0};
  let now=new Date().toISOString(),payload={status,next_follow_up_at:null,updated_at:now};
  let{data,error}=await sb.from('leads').update(payload).eq('customer_id',customerId).select('id');
  if(error){console.warn('Lead lifecycle sync:',error);return {ok:false,error}}
  return {ok:true,count:(data||[]).length};
}
async function syncQuoteLifecycle(quoteId,quoteStatus,leadStatus){
  if(!quoteId)return {ok:false};
  let{data:q,error}=await sb.from('quotes').select('id,customer_id,status').eq('id',quoteId).single();
  if(error){console.warn('Quote lifecycle lookup:',error);return {ok:false,error}}
  if(quoteStatus&&q.status!==quoteStatus){
    let r=await sb.from('quotes').update({status:quoteStatus,updated_at:new Date().toISOString()}).eq('id',quoteId);
    if(r.error){console.warn('Quote lifecycle sync:',r.error);return {ok:false,error:r.error}}
  }
  if(leadStatus&&q.customer_id)await syncCustomerLifecycle(q.customer_id,leadStatus);
  return {ok:true,customer_id:q.customer_id};
}
async function lifecycleAfterInvoice(quoteId){return syncQuoteLifecycle(quoteId,'Approved','approved')}
async function lifecycleAfterSchedule(quoteId,status){
  if(status==='Completed')return syncQuoteLifecycle(quoteId,'Completed','completed');
  if(status==='Canceled')return syncQuoteLifecycle(quoteId,'Approved','approved');
  return syncQuoteLifecycle(quoteId,'Scheduled','scheduled');
}
...
function renderQuoteResults(){
  let data=window._cloudQuotes||[],
      ...
      status=$('quoteStatusFilter')?.value||'active',
      filtered=data
        .filter(x=>{let archived=['Completed','Lost'],statusMatch=status==='active'?!archived.includes(x.status):status==='archived'?archived.includes(x.status):!status||x.status===status;return statusMatch&&JSON.stringify(x).toLowerCase().includes(q)})
    ...
    return `<details class="quote-index-card" data-quoteindex="${q.id}">
      ...
      <div class="quote-index-expanded">
        <div class="quote-index-top-actions">
          ${phone?`<a class="btn primary quote-call-btn" href="tel:${esc(phone)}">Call Customer</a><a class="btn" href="sms:${esc(phone)}">Text Customer</a>`:''}
...
async function openScheduleJob(quoteId){$('scheduleMessage').textContent='Loading installers…';let quote=window._cloudQuotes?.find(q=>q.id===quoteId);if(!quote){let{data,error}=await sb.from('quotes').select('*,customer:customers(first_name,last_name)').eq('id',quoteId).single();if(error)return toast(error.message);quote=data}let[{data:people,error:peopleError},{data:existing,error:jobError}]=await Promise.all([sb.from('profiles').select('id,full_name,email,role,active').eq('active',true).in('role',['installer','manager','owner']).order('full_name'),sb.from('jobs').select('*').eq('quote_id',quoteId).order('created_at',{ascending:false}).limit(1).maybeSingle()]);if(peopleError||jobError)return toast((peopleError||jobError).message);if(!people?.length)return toast('No active installer accounts were found.');let selected=existing?.assigned_installers?.length?existing.assigned_installers:(existing?.assigned_to?[existing.assigned_to]:[people.find(p=>String(p.role)==='installer')?.id||people[0].id]);$('scheduleInstallers').innerHTML=(people||[]).map(p=>`<label class="installer-option"><input type="checkbox" value="${p.id}" ${selected.includes(p.id)?'checked':''}><span>${esc(p.full_name||p.email)}<small>${esc(String(p.role))}</small></span></label>`).join('');$('scheduleQuoteId').value=quoteId;$('scheduleQuoteName').textContent=`${quote.customer?.first_name||''} ${quote.customer?.last_name||''} • ${quote.project_name||'Project'}`.trim();$('scheduleTitle').value=existing?.title||`${quote.customer?.last_name||quote.customer?.first_name||'Customer'} — ${quote.project_name||(/Cleaning$/.test(quote.project_type||'')?'Window Cleaning':'Window Film Installation')}`;$('scheduleNotes').value=existing?.notes||quote.notes||'';$('scheduleStatus').value=existing?.status||'Scheduled';scheduleBindPickers();scheduleSetPicker('scheduleStart',existing?.scheduled_start||null);scheduleSetPicker('scheduleEnd',existing?.scheduled_end||null);$('scheduleJobModal').dataset.jobId=existing?.id||'';$('scheduleJobModal').dataset.originalStatus=existing?.status||'Scheduled';$('scheduleMessage').textContent=existing?'This quote already has an assigned job. Saving will update it.':'Choose one or more installers and the installation time.';$('scheduleJobModal').querySelector('.job-material-card')?.classList.toggle('hidden',/Cleaning$/.test(quote.project_type||''));$('scheduleJobModal').classList.add('show');if(!/Cleaning$/.test(quote.project_type||''))loadScheduleMaterialPlan(quote,existing?.id||'')}function toLocalInput(value){if(!value)return'';let d=new Date(value),off=d.getTimezoneOffset();return new Date(d.getTime()-off*60000).toISOString().slice(0,16)}async function saveScheduledJob(){
  ...
  await lifecycleAfterSchedule(quoteId,selectedStatus);
